...
const TradingDashboard: React.FC = () => {
  const { signals, positions, marketData, performance, priceHistory } = useWebSocketContext();
                  ...
                  >
                    <div className="mb-3">
                      <div className="flex justify-between items-start">
                        ...
                        >
                          {isUp ? '↑' : '↓'}{' '}
                          {Math.abs(data?.signed_change_rate * 100 || 0).toFixed(2)}%
                        </span>
                      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {positions.filter((p) => p.status === 'OPEN').length === 0 && (
              <div className="col-span-full text-center py-8 text-gray-500">No open positions</div>
            )}
          </div>
        </div>
      </div>
