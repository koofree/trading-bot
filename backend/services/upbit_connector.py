...
class UpbitConnector:
    ...
    def get_candles(
        self, market: str, interval: str = "minutes", unit: int = 1, count: int = 200
    ) -> List[Dict]:
        """
    ...
    def get_orders(
        ...
        response = requests.get(
            f"{self.base_url}/orders", params=query, headers=headers
        )
        return response.json()
    ...
    def place_order(
        ...
        response = requests.post(f"{self.base_url}/orders", data=data, headers=headers)
    ...
    def cancel_order(self, uuid: str) -> Dict:
        ...
        response = requests.delete(
            f"{self.base_url}/order", params=data, headers=headers
        )
        return response.json()
