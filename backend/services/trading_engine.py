...
class TradingEngine:
    ...
    async def update_balance(self):
        ...
        try:
            ...
            if isinstance(accounts, list) and accounts:
                self.balance = {
                    acc["currency"]: float(acc["balance"]) for acc in accounts
                }
    ...
    async def execute_signal(self, signal) -> Dict:
        ...
        try:
            ...
            if signal.signal_type.value == "BUY":
                ...
                return {"status": "hold", "reason": "Hold signal - no action taken"}
    ...
    async def _open_position(self, signal) -> Dict:
        ...
        try:
            ...
            if order_result.get("status") == "success":
                ...
                logger.info(
                    f"Position opened: {position.position_id} for {signal.market}"
                )
    ...
    async def _close_position(self, signal, position: Position) -> Dict:
        ...
        try:
            ...
            if order_result.get("status") == "success":
                ...
                logger.info(
                    f"Position closed: {position.position_id} with P&L: {position.pnl:.2f}"
                )
    ...
    async def _check_risk_limits(self, signal) -> bool:
        ...
        if self.risk_metrics.daily_pnl < -(
            self.balance.get("KRW", 0) * self.daily_loss_limit
        ):
        ...
        if self.balance.get("KRW", 0) < required_balance:
            logger.warning(
                f"Insufficient balance: {self.balance.get('KRW', 0)} < {required_balance}"
            )
            return False
                ...
                for position_id, position in list(self.positions.items()):
                    ...
                    if ticker:
                        ...
                        # Check take-profit
                            ...
                            await self._close_position_market(
                                position, "Take-profit triggered"
                            )
    ...
    def _update_risk_metrics(self, position: Position):
        ...
        self.risk_metrics.win_rate = (
            len(wins) / len(total_closed) if total_closed else 0
        )
    ...
    async def _set_stop_loss(self, position: Position):
        ...
        logger.info(
            f"Stop-loss monitoring activated for {position.position_id} at {position.stop_loss}"
        )
    ...
    async def _set_take_profit(self, position: Position):
        ...
        # This would need to be implemented as a monitoring system
        logger.info(
            f"Take-profit monitoring activated for {position.position_id} at {position.take_profit}"
        )
