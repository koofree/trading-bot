...
class DocumentProcessor:
        ...
        try:
            if file_type == ".pdf":
                ...
                return await asyncio.to_thread(
                    self._extract_spreadsheet_data, file_path
                )
            elif file_type == ".json":
    ...
    def _extract_key_information(self, text: str) -> Dict:
        ...
        extracted["tickers"] = list(
            set(t for t in ticker_matches if t not in common_words)
        )[:20]
        ...
        extracted["support_resistance"] = [
            self._clean_number(sr) for sr in sr_matches[:10]
        ]
    ...
    async def batch_process_documents(self, file_paths: List[str]) -> List[Dict]:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                processed.append({"error": str(result), "file_path": file_paths[i]})
            else:
