...
class PositionRepository:
    ...
    def create(db, position_data: dict) -> Position:
        ...
        # Cache position
        RedisCache.set_position(position.position_id, {
            'id': position.id,
    ...
    @staticmethod
    def update_pnl(db, position_id: str, current_price: float, pnl: float, pnl_percentage: float):
        """Update position P&L"""
...
class PerformanceRepository:
    ...
    def update_daily_metrics(db, metrics: dict):
        ...
        existing = db.query(PerformanceMetric).filter(
            func.date(PerformanceMetric.date) == today
        ).first()
    ...
    def get_metrics(db, days: int = 30):
        ...
        return db.query(PerformanceMetric).order_by(
